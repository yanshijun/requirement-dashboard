...
function genId() {
  return "rbug_" + Date.now() + "_" + Math.random().toString(36).slice(2, 6);
}
...
exports.handler = async function (event) {
  ...
  try {
    ...
    if (event.httpMethod === "GET" || action === "list") {
      ...
      let sql = "SELECT * FROM requirement_bugs WHERE 1=1";
      const params = [];
    ...
    if (action === "add") {
      ...
      const id = genId();
      await pool.execute(
        "INSERT INTO requirement_bugs (id,product,module,title,description,attachments,severity,status,reporter) VALUES (?,?,?,?,?,?,?,?,?)",
    ...
    if (action === "update") {
      ...
      if (!title) return err("Bug标题不能为空", 400);
      await pool.execute(
        "UPDATE requirement_bugs SET product=?,module=?,title=?,description=?,attachments=?,severity=?,status=?,reporter=?,updated_at=NOW() WHERE id=?",
    ...
    if (action === "delete") {
      const { id } = body;
      if (!id) return err("缺少 id", 400);
      await pool.execute("DELETE FROM requirement_bugs WHERE id=?", [id]);
      return ok({ ok: true });
    }

    return err("未知操作: " + action, 400);
  } catch (e) {
    console.error("[reqbugs] error:", e);
